...
public class Deposit {
    ...
    private DateTimeFormatter form = DateTimeFormatter.ofPattern("MM/DD/YYY");
    ...
    public static void createNewDeposit(String accountID, LocalDate date, String amount){
        connection = ConnectDB.setupConnection();
        int count = 0;

        try{
            String query = String.format("INSERT INTO depositewithdraw(accountid, actiontype, dwDate, amount) VALUES('%s', '%c', STR_TO_DATE('%s', %%m/%%d/%%y), %f);", accountID, 'd', date, amount);
